...
function ViewStarsButton_Activated( button )
{
	...
	if ( level.ui.nextMapModeComboIndex == null || GetConVarBool( "hide_server" ) == false )
		return
...
function UpdateLobbyTitle()
{
	...
	while ( 1 )
	{
		local currentHideServer = GetConVarBool( "hide_server" )
...
function UpdateLobbyType()
{
	...
	local lastHideServer = GetConVarBool( "hide_server" )
	...
	while ( 1 )
	{
	    ...
	    if ( currentHideServer != lastHideServer )
	    {
