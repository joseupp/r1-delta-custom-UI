	...
	while ( 1 )
	{
		...
		if ( uiGlobal.canSetDataCenter != lastResult )
		{
			if ( uiGlobal.canSetDataCenter )
			{
				...
				RegisterButtonPressedCallback( BUTTON_B, OpenOfflineNameDialogButton_Activate )
			}
			else
			{
				DeregisterButtonPressedCallback( BUTTON_X, DataCenterDialog )
				DeregisterButtonPressedCallback( BUTTON_Y, OnAddonButton_Activate )
				DeregisterButtonPressedCallback( BUTTON_B, OpenOfflineNameDialogButton_Activate )
			}
		}
...
function SwitchProgression()
{
	local lastResult = false
	local mainMenu = GetMenu( "LobbyMenu")
	...
	while ( 1 )
	{
		...
		uiGlobal.canSetDataCenter = uiGlobal.activeMenu == mainMenu && uiGlobal.mainMenuFocus != null && uiGlobal.activeDialog == null
