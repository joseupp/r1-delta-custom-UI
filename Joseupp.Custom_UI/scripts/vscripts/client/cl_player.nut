...
// Dont enable, breaks flyout if the weapon has unknown mods
...
function ServerCallback_OnEntityKilled( attackerEHandle, victimEHandle, scriptDamageType, damageSourceId )
{
    ...
    // killsound
	if( victim.IsPlayer() && victim != attacker || victim.IsSpectre() || victim.IsSoldier() )
	{
	...
	//killsound npcsoldier Disable for now
/*	if ( victim.IsSoldier() && victim != attacker)
	{
		if( attacker == localPlayer )
		{
			EmitSoundOnEntity( attacker, "PlayerJoinedLobby" )
